...
Setup first: accept the GitHub invite → `git clone https://github.com/danielpodolsky-collab/dropin && cd dropin && claude` → `/model` = Fable 5.1 → paste.
...
**1. Server skeleton (first PR, merged by 19:35).** `server/`, FastAPI + uv, port 8000, CORS open to `http://localhost:5173`. In-memory job dict, one background task per job. No DB, queue, S3, WebSockets, Docker, auth, tests. One run command, written in `server/README.md`.
...
## Clock
19:35 server PR merged (tell Daniel the run command) · 19:45 full flow works on `main` · **19:50 freeze — fixes only** · then Phase 2 arrives from Daniel.
